...
cond<-matrix(c("E","E","E"), nitems, 3, byrow=T) 
...
for(i1 in c(1:nitems)){
	...
	tmod2<- glm(Acc~Pic, data = sdat[[3]], family="binomial") 	#fit test 2 model
	t1[i1,]<-tmod1$coefficients 					#store test 1 coefficients
	t2[i1,]<-tmod2$coefficients					#store test 2 coefficients
	cond[i1,]<-as.matrix((sdat[[1]])[1,c(5,6,3)])			#pull out other info about this sj and condition
...
names(out)<-c("axlab","farsource","sno","t1int","t1slope","t1bound","t2int","t2slope","t2bound") #name columns

bshift <- b2 - b1		#compute shift in boundary
closepos <- bshift		#copy bshift to midshift

#Flip direction for people in far-right group
#so positive numbers mean shift toward close source
#and away from far source
closepos[out$farsource=="R"] <- closepos[out$farsource=="R"] * -1

out<-cbind(out, bshift, closepos) #glue boundary shift data to output dataframe

Zclosepos <- (out$closepos - mean(out$closepos, na.rm=T))/sqrt(var(out$closepos, na.rm=T))  #compute Zscore for closepos across all conditions
...
#       }
out<-cbind(out, Zclosepos) #glue Zscores onto ouput data frame
out #output result
}